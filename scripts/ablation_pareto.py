#!/usr/bin/env python3
"""Epsilon Pareto demo: update rate falls and output error rises with gate threshold.
...
def main():
    ...
    print(f"{'eps':>6} | {'upd_rate':>8} | {'output error':>22} | {'err/eps':>8}")
    rows = []
    for eps in eps_grid:
        set_eps(ms, eps)
        with torch.no_grad():
    ...
    print(f"\nupdate rate falls with eps: {ur_mono}   error rises with eps: {err_mono}")
    ...
    try:
        ...
        plt.xlabel("update rate"); plt.ylabel("output error"); plt.title("eps-Pareto")
        plt.tight_layout(); plt.savefig("ablation_pareto.png", dpi=120)
        print("  saved ablation_pareto.png")
    except Exception:
        pass
